...
export default function MintPage() {
  ...
  const handleExecuteMint = async () => {
    ...
    try {
      ...
      const res = await fiatApi.postOnRamp(fiatAmount, selectedFiatCurrency, opts);
      setTxId(
    ...
    <>
      ...
      <PageContainer>
        ...
        <Tabs
          defaultValue="mint"
          className="w-full"
          onValueChange={(v) => setActiveTab(v as "mint" | "burn" | "rates")}
        >
          ...
          <TabsContent value="mint" className="py-6 space-y-4">
            <MintForm
              ...
              onCurrencyChange={setSelectedFiatCurrency}
              fiatAmount={fiatAmount}
              onFiatAmountChange={setFiatAmount}
              estimatedMintAcbu={estimatedMintAcbu}
              mintError={mintError}
      ...
      <AlertDialog open={step === "confirm"}>
        <AlertDialogContent className="max-w-md border-border">
          <AlertDialogHeader>
            <AlertDialogTitle>
              {activeTab === "mint" ? "Confirm Mint" : "Confirm Burn"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {activeTab === "mint" &&
      ...
      <AlertDialog open={step === "success"}>
        <AlertDialogContent className="max-w-md border-border">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Your {activeTab} operation has been submitted successfully.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4 space-y-2">
            ...
            {activeTab === "mint" && (
              <p className="text-xs text-muted-foreground">
                If the Stellar mint contract errors, tokens are still recorded in
                the app; your balance may show &quot;app ledger&quot; until
                on-chain mint works.
              </p>
            )}
          </div>
